...
import { FiRefreshCw, FiSave, FiShuffle } from 'react-icons/fi'
import { adminGetRoutingRules, adminSetRoutingRules } from '../services/admin.api'
...
export default function AdminRoutingRules() {
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      const data = await adminGetRoutingRules()
      setRaw(JSON.stringify(data?.config || {}, null, 2))
    } catch (e) {
      setError(e?.response?.data?.error || e?.message || 'Failed to load routing rules')
    } finally {
      setLoading(false)
    }
  }
  ...
  const save = async () => {
    ...
    try {
      ...
      await adminSetRoutingRules(parsed)
      await load()
    } catch (e) {
      setError(e?.response?.data?.error || e?.message || 'Failed to save routing rules')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="card p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Queues & routing rules</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Define default routing, auto-assignment modes, and escalation rules (v1 JSON config).
            </p>
          </div>
