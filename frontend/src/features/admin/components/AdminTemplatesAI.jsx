...
import { FiRefreshCw, FiSave, FiTool } from 'react-icons/fi'
import { adminGetTemplatesConfig, adminSetTemplatesConfig } from '../services/admin.api'

export default function AdminTemplatesAI() {
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      const data = await adminGetTemplatesConfig()
      setRaw(JSON.stringify(data?.config || {}, null, 2))
    } catch (e) {
      setError(e?.response?.data?.error || e?.message || 'Failed to load templates config')
    } finally {
      setLoading(false)
    }
  }
  ...
  const save = async () => {
    ...
    try {
      ...
      await adminSetTemplatesConfig(parsed)
      await load()
    } catch (e) {
      setError(e?.response?.data?.error || e?.message || 'Failed to save templates config')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="card p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Templates & AI controls</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Centralize reply templates, AI defaults, and guardrails (stored as JSON).
            </p>
          </div>
