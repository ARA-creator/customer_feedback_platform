...
import { adminGetScoringConfig, adminSetScoringConfig } from '../services/admin.api'
...
export default function AdminScoring() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(e?.response?.data?.error || e?.message || 'Failed to load scoring config')
    } finally {
      setLoading(false)
    }
  }
  ...
  const save = async () => {
    ...
    try {
      ...
      await adminSetScoringConfig(parsed)
      await load()
    } catch (e) {
      setError(e?.response?.data?.error || e?.message || 'Failed to save scoring config')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="card p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Scoring configuration</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Tune prioritization weights and urgency rules (stored in the database as JSON).
            </p>
          </div>
